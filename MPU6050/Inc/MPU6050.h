...
#define I2C1_RX_DMA_BUFF_SIZ	14

typedef enum AccelRange_t
{
	ACC_2G,
	ACC_4G,
	ACC_8G,
...
{		
	...
	uint8_t dev_address;   		// [address] << 1 を格納すること
	
	uint8_t accel_scale; 		// 0 = ±2g, 1 = ±4g, 2 = ±8g, 3 = ±16g
	uint8_t gyro_scale; 		// 0 = ±250deg/s, 1 = ±500deg/s, 2 ±1000deg/s, 3 = ±2000deg/s
	
	uint16_t gyro_offset_calc_number;
		
	uint8_t rawdata_buff[I2C1_RX_DMA_BUFF_SIZ];    		//gyro, accでそれぞれ6byteずつ,temperatureで2byteで計14byte
	...
	float gyro_raw_offset[3];
	float accel_raw_offset[3];
...
void MPU6050_SetGyroOffsetCalcSampleNumber(MPU6050_t* const mpu6050, uint16_t number);
